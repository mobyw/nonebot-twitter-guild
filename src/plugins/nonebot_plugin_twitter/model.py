...
def AddNewUser(screen_name: str, name: str, id: str):  # 创建用户对应的表
    ...
    if CUR.fetchall()[0][0] == 0:
        ...
        CUR.execute(
            'insert into user_list values("{}","{}","{}","")'.format(
                screen_name, name, id
            )
        )
        DB.commit()
...
def AddCard(screen_name: str, ID: str, cid: int) -> int:  # 添加订阅信息 返回类型 记录是否已存在(int)1：存在
    ...
    if CUR.fetchall()[0][0] != 0:
        ...
        return 1
    CUR.execute(
        'insert into {} values("{}",{},{})'.format(table_name, ID, str(cid), str(0))
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def TranslateON(screen_name: str, ID: str, cid: int):  # 开启推文翻译
    ...
    table_name = "_" + screen_name
    CUR.execute(
        'update {} set translate=1 where id="{}" and cid={}'.format(table_name, ID, cid)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def TranslateOFF(screen_name: str, ID: str, cid: int):  # 关闭推文翻译
    ...
    table_name = "_" + screen_name
    CUR.execute(
        'update {} set translate=0 where id="{}" and cid={}'.format(table_name, ID, cid)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def UpdateTweet(screen_name: str, tweet_id: str):  # 更新用户最新推文ID
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update user_list set tweet_id="{}" where screen_name="{}"'.format(
